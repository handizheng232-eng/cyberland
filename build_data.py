...
"forecast_unit": "万吨 SC6 折算（原 dmt，品位 ~SC5.2——FY27 官方指引品位假设；逐季 grade produced 5.1-5.3%；FY27 指引 1,030-1,100kt dmt（财年口径 2026-07至2027-06）→ SC6 893-953kt；日历 2027 预测=FY27 指引覆盖前两季+FY28 外推后两季；双厂 dmt 产能 ~1.25Mtpa → SC6 ~1.1Mt，利用率 83-88%）",
        "forecast_2027": {
            ...
            "scenarios": {
                "bear": {"label": "悲观（Ngungaju 爬坡慢于计划 + 锂价回落压缩产量）", "production_kt": 1040, "note": "Ngungaju 爬坡慢、Pilgan 利用率回落；全年 ~104 万吨 dmt → SC6 折算 ~91.8（×0.883，品位 ~SC5.2）"},
                "base": {"label": "基准（日历 2027：FY27 指引覆盖 H1 + FY28 外推 H2）", "production_kt": 1080, "note": "Pilgan ~210kt/季×4 + Ngungaju 达产后 ~55-60kt/季×4 → ~108 万吨 dmt → SC6 折算 ~95.4（×0.883，品位 ~SC5.2；对应 FY27 指引上沿）"},
                "bull": {"label": "乐观（锂价大涨 + 双厂超产运行）", "production_kt": 1100, "note": "双厂超产运行 → ~110 万吨 dmt → SC6 折算 ~97.1（×0.883，品位 ~SC5.2；P2000 仅 pre-FID、FID 2026-12 未定，不进入 2027——审计修正）"}
            },
...
FC_BASIS = {
    ...
    "Pilgangoora": {
        "basis": "FY27 指引（2026-07至2027-06 财年）1,030-1,100kt（中值 106.5）→ 季均 26.6，覆盖日历 26Q3-27Q2 四季；日历 2027 = FY27 后两季（27Q1/27Q2=26.6，指引覆盖）+ FY28 前两季（27Q3/27Q4——FY28 无官方指引，按 Ngungaju 满产+Pilgan 稳态、产能利用率 ~86-88% 外推 27.5/季）→ 日历 2027 基准 108.2 万吨 dmt（指引覆盖 53.2 + 外推 55.0）→ SC6 折算 95.5（×0.883，品位 ~SC5.2）；26Q2 实际 23.24（FY26Q4）→ 26Q3E=24.0 线性爬坡。",
