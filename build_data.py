        ...
        "forecast_2027": {
            ...
            "scenarios": {
                ...
                "base": {
                    ...
                    "production_kt": 1740,
                    "note": "CGP3 2027 年初爬至 ~85% 利用率、年中满产，全年贡献 ~47 万吨；存量 ~130 万吨；合计 ≈ 174 万吨。"
...
"forecast_unit": "万吨 dmt（实际品位 ~5.2% SC；FY27 指引 1,030-1,100kt 对应）",
        "forecast_2027": {
            ...
            "scenarios": {
                "bear": {"label": "悲观（Ngungaju 爬坡慢于计划 + 锂价回落压缩产量）", "production_kt": 950, "note": "Ngungaju 2027 年中才达产、Pilgan 利用率回落；全年 ~95 万吨"},
                "base": {"label": "基准（双厂满产，FY27 指引兑现）", "production_kt": 1080, "note": "Pilgan ~210kt/季×4 + Ngungaju 达产后 ~55-60kt/季×4 → ~108 万吨（对应 FY27 指引上沿）"},
                "bull": {"label": "乐观（锂价大涨 + P2000 提前 FID 动工）", "production_kt": 1180, "note": "双厂超产 ~110 万吨 + P2000 建设期效率提升 → ~118 万吨"}
            },
    ...
    },
    {
        ...
        "fc_2027": [
            ...
            {"label": "基准", "val": 80, "note": "三线满产 + Stage 4 按时供矿（750ktpa SC5.5% 产能 → 混合品位 ~80 万吨/年）"},
    ...
    },
    {
        ...
        "fc_2027": [
            ...
            {"label": "基准", "val": 66, "note": "DMS 满产 + 浮选厂 2027 年中投产（FY26 销量 242k SC6 100% 口径 → 年化 ~66 万吨混合品位）"},
            {"label": "乐观", "val": 76, "note": "浮选厂提前投产 + 地下开发贡献 + 回收率升至 63%+"}
        ],
        "capacity_verification": {
            "title": "选矿产能核实（多来源交叉印证）",
...
,
    {
        ...
        "fc_2027": [
            {"label": "悲观", "val": 40, "note": "地下爬坡不及预期 + 回收率维持 63%（FY27 指引下沿 390k SC6）"},
            {"label": "基准", "val": 44, "note": "FY27 指引中值 415k SC6（390-440k）+ 2.8Mtpa 原矿运行率 FY27 底达成"},
                ...
                "items": [
                    {
                        ...
                        "sources": [
                            ...
                            {"src": "Q4 FY26 季报", "data": "26Q2 45% 露天矿入厂（收尾阶段）"}
                        ]
                    },
                    {
                        "line": "地下矿（澳洲首个地下锂矿）",
            ...
            "mining_side": {
                ...
                "items": [
                    ...
                    },
                    {
                        ...
                        "sources": [
                            {"src": "Arcadium FY2024 10-K（最新，2024-12-31，S-K1300）", "data": "资源 M&I 6.48Mt @ 1.41% Li₂O + Inferred 4.81Mt @ 1.27%（COG 0.3% 露天 / 0.58% 地下）；储量 P&P 3.73Mt @ 1.02%（Proven 89kt + Probable 3,644kt）——较 2023-06 的 12.1Mt 大幅下降（开采消耗 + 口径调整）"}, {"src": "Allkem NI 43-101（2023）", "data": "历史资源 12.1Mt @ 1.3% Li₂O + 167ppm Ta₂O₅（2023-06-30，COG 0.3%）；储量 7.1Mt @ 1.2% Li₂O（2023，Proven 0.2 + Probable 5.2 + 堆存 1.8）"},
...
for q, p, c in [
    ...
    ("2025Q4", 1161, 585),   # PLS Mar2026 QAR 对比列: US$1,161/t (SC5.2 CIF), A$585 FOB
...
for q, p, s in [
    ...
    ("2026Q1", 16.0, 14.2),      # Q3FY26: 80k→100%≈160k / 71k→100%≈142k
    ("2026Q2", 16.4, 18.8),      # Q4FY26: 82k→100%≈164k / 94k→100%≈184k
...
# FY25 官方全年（2024Q3-2025Q2）：产 63k mixed / 54k SC6、销 70k / 59k → 季度估算（标注估算）
for q, p, s in [
    ("2024Q3", 1.6, 1.8),       # FY25 Q1 估算（全年 63k/70k ÷ 4）
    ("2024Q4", 1.6, 1.8),       # FY25 Q2 估算
    ("2025Q1", 1.6, 1.8),       # FY25 Q3 估算
    ("2025Q2", 1.6, 1.8),       # FY25 Q4 估算
...
for q, pr, c in [
    ("2024Q3", 808, 1148),   # FY25 全年均价 US$808 SC6 / FOB A$1,148（2024Q3-2025Q2 期间）
    ("2024Q4", 808, 1148),
    ("2025Q1", 808, 1148),
    ("2025Q2", 808, 1148),
    # 2025Q3-2026Q2 停产/复产期无销售 → 无价格/成本
]:
...
for q, p, s in [
    ...
    ("2023Q3", 3.3, 3.3),
    ("2023Q4", 3.3, 3.3),
    ("2024Q1", 2.7, 2.7),       # Arcadium FY2024 销量 ~140kt（Q1+Q2 合并 53.5kt 未拆分 → 均分估算）
    ("2024Q2", 2.7, 2.7),
    ("2024Q3", 3.2, 3.2),       # Arcadium 10-K 销量：2024Q3 ≈32.4kt（产量近似销量）
    ("2024Q4", 5.4, 5.4),       # Arcadium 10-K 销量：2024Q4 ≈54.1kt
    # 2025 年：2025 H1 底前完成 Stage 3 后进入 C&M（Arcadium 2024-09-04 公告；力拓官网确认），2025Q3 起停产
    ("2025Q3", 0.0, 0.0),       # 2025 年进入 C&M（力拓官网确认，因锂价低）
    ("2025Q4", 0.0, 0.0),
    ("2026Q1", 0.0, 0.0),
    ("2026Q2", 0.0, 0.0),
]:
...
for q, p, s in [
    ...
    ("2026Q2", 0.0, 0.0),       # 2026-05 恢复采矿（Grants）；首产精矿目标 2026 年 9 月季度（26Q3）——26Q2 无精矿产出
]:
    fn_set(fn_prod, q, p)
    fn_set(fn_sales, q, s)
...
CAPEX = {
    ...
    "Wodgina": {"2025": 1.35, "2026E": 1.60, "unit": "MRL 披露；2026E=1H26 $80M×2 年化；三线+POSCO 30% 转让后口径"},
