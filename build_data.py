...
"forecast_unit": "万吨 SC6（6% Li₂O 折算；IGO FY27 指引 155-175 万吨对应）",
    ...
    },
    {
        ...
        "grade_series": {"2026Q2": 5.3},   # 逐季品位（2026-08-11 机制：报告披露每季平均品位时逐季折算；PLS 其余季度 QAR 品位未逐一获取，按统一 5.3%）（用户规范 2026-08-11：产量/销量描述必须明确品位）
...
"forecast_unit": "万吨 dmt（实际品位 ~5.3% SC；FY27 指引 1,030-1,100kt 对应）",
        "forecast_2027": {
            ...
            "scenarios": {
                "bear": {"label": "悲观（Ngungaju 爬坡慢于计划 + 锂价回落压缩产量）", "production_kt": 1040, "note": "Ngungaju 爬坡慢、Pilgan 利用率回落；全年 ~104 万吨（线性覆写口径，审计修正统一）"},
                "base": {"label": "基准（双厂满产，FY27 指引兑现）", "production_kt": 1080, "note": "Pilgan ~210kt/季×4 + Ngungaju 达产后 ~55-60kt/季×4 → ~108 万吨（对应 FY27 指引上沿）"},
                "bull": {"label": "乐观（锂价大涨 + 双厂超产运行）", "production_kt": 1100, "note": "双厂超产运行 → ~110 万吨（P2000 仅 pre-FID、FID 2026-12 未定，不进入 2027——审计修正）"}
            },
...
for q, p, s in [
    ...
    ("2025Q1", 11.04, 11.09),
    ("2025Q2", 18.81, 18.36),
    ("2025Q3", 22.13, 21.60),   # PLS FY26Q1 (Sep2025 QAR): 221.272kt / 215.982kt（官方 2026-07-30 季报核实）
    ("2025Q4", 22.48, 21.40),   # PLS FY26Q2 (Dec2025 QAR): 224.757kt / 214.025kt（官方）
    ("2026Q1", 20.80, 23.20),   # PLS FY26Q3 (Mar2026 QAR): 208.022kt / 231.971kt（官方）
    ("2026Q2", 23.24, 24.99),   # PLS FY26Q4 (Jun2026 QAR): 232.436kt / 249.905kt（官方）
...
FC_BASIS = {
    ...
    "Pilgangoora": {
        "basis": "FY27 官方指引 103-110 万吨（中值 106.5）→ 季均 26.6；26Q2 实际 23.24 dmt（FY26Q4 官方；Ngungaju 2026-07 重启后双厂爬坡中）→ 线性 26Q3E=24.0、26Q4E=26.6；27Q1/27Q2=26.6、27Q3/27Q4 线性至满产 27.5（双厂 ~1.25Mtpa）；2027 基准=93.6 万吨 SC6（108.2 dmt ×0.867）。",
